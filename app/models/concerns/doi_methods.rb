...
module DoiMethods
  ...
  def set_dataset_doi
    doi = (self.publication.try(:first).try(:hasDocument).try(:first).try(:doi) ? self.publication.first.hasDocument.first.doi.first
    : DoiGenerator.generate(self.noid) )
  ...
  self.publication[0].hasDocument[0].doi = doi
end
