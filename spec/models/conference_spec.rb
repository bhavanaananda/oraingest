...
describe Conference do
  ...
  describe 'attributes' do
    ...
    context 'ConferenceRdfDatastream' do
      it { is_expected.to respond_to(:time) }
      it { is_expected.to respond_to(:season) }
      it { is_expected.to respond_to(:place) }
      it { is_expected.to respond_to(:wasAssociatedWith ) }
      it { is_expected.to respond_to(:conferenceAssociation) }
      it { is_expected.to respond_to(:conferenceName) }
      it { is_expected.to respond_to(:conferenceAbbreviation) }
      it { is_expected.to respond_to(:conferenceHomepage) }
      it { is_expected.to respond_to(:conferenceProceeding) }
      it { is_expected.to respond_to(:hadCreationActivity) }
    ...
    # end
    # context 'ConferenceAdminRdfDatastream' do
  ...
  describe 'when creating a new conference' do
    ...
    it 'initializes the submission workflow' do
      @conference.save
      expect(@conference.workflows).not_to be_empty
    end

    it 'removes blank assertions' do
      @conference.title = 'Test title'
      @conference.subtitle = ''
      @conference.save
      expect(@conference.title).to eq(['Test title'])
      expect(@conference.subtitle).to eq([])
      expect(@conference.keyword).to eq([])
    end
  end
  ...
  describe 'applying permissions' do
    before do
      ...
      @conference.apply_permissions(@reviewer)
    end

    it 'should set the permisions' do
      expect(@conference.permissions).not_to be_empty
    end

    it 'sets the permissions to reviewer/group/edit' do
      permission = @conference.permissions.first
  ...
  describe '#to_jq_upload' do
    before do
      @conference = Conference.new
      @jq_upload = @conference.to_jq_upload('title', 120, 'uuid:nn999n999', 'dsid')
    end
    ...
    it 'sets the url' do
      expect(@jq_upload['url']).to eq('/conferences/uuid:nn999n999/file/dsid')
    end
    ...
    it 'sets the delete url' do
      expect(@jq_upload['delete_url']).to eq('/conferences/uuid:nn999n999/file/dsid')
    end
